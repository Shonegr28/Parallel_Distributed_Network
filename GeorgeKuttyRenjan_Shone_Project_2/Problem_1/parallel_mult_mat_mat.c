...
int main(int argc, char* argv[])
{
    ...
    for (int j = 0; j < n_col2; j++)
    {
        ...
        {
            long int sum = 0; // stores dot product of cur-row-matrix1 and cur-col-matrix2
            // iterate through the row of matrix 1 and column of matrix 2 to compute dot product
    ...
    // iterate through each row of the output matrix
    for (int i = 0; i < n_row1; i++) {
        // iterate through each column of the output matrix
        for (int j = 0; j < n_col2; j++){
            // write the value to the file
            fprintf(outputFile, "%ld", resultMatrix[(long int)i * (long int)n_col2 + (long int)j]);

            if (j < n_col2 - 1) fprintf(outputFile, ",");
        }
        fprintf(outputFile, "\n");
    }
